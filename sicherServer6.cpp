...
#define PROTOKOLL_FAMILIE AF_INET6
//#define PROTOKOLL_FAMILIE AF_INET
...
int fahreHoch(u_short *port)
{
    ...
    acceptPort = socket(PROTOKOLL_FAMILIE, SOCK_STREAM, 0);
    if (acceptPort == -1)
    {
      fatalerFehler("socket");
    }
    memset(&name, 0, sizeof(name));
    name.sin6_family = PROTOKOLL_FAMILIE;
...
int main(void)
{
 ...
 u_short port = 8000;
 int client_sock = -1;
 //struct sockaddr_in6 name, cli_addr;
